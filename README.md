...
- 场景提示词数量多，逐条手动提交生图任务效率低，容易漏项和错配；
- 生图任务需要统一处理异步轮询、依赖调度、失败阻塞和参考图复用等工程逻辑；
- 测试方案的质量、Token、缓存、耗时和费用需要能够横向比较。

## 项目背景与演进目标

抖音近期兴起的“人生副本”类视频，将真实或虚构的人生经历转化为连续叙事、视觉分镜和旁白内容，具有较强的故事性和传播性。为了探索这类内容的自动化生产方式，我从最初编写提示词和 Skill 入手，逐步搭建文本处理工作流、生图工作流和可视化工作台，将原本需要人工逐段处理的创作过程拆解为多个可验证、可恢复的阶段。

当前系统已经能够将中文人生故事转换为结构化场景、连续视觉分镜、场景级生图提示词和参考图规划，并进一步调度生图任务。后续计划将其演化为端到端的自动化内容生产系统：

1. 通过定时任务获取抖音热点数据，搜索“人生副本”等相关关键词；
2. 对候选视频的播放量、点赞量、评论量等数据进行比较，选择具有代表性的选题；
3. 自动提取公开素材的字幕，分析其叙事结构和内容特点；
4. 使用仿写与改写模块提炼选题方向，在保留主题价值的基础上生成原创人生副本故事；
5. 将原创故事送入文本流水线，完成语义分块、场景拆分、人物设定、提示词生成和质量验收；
6. 将验收通过的结构化结果交给生图流水线，生成角色参考图和场景图片；
7. 使用语音模型生成旁白，并组织配音、字幕、画面、片头和背景音乐；
8. 自动合成完整视频，并在内容质量检查后发布到目标视频平台。

最终目标是形成一条从热点发现、选题分析、原创写作、分镜生成、生图、配音、视频合成到发布的自动化内容生产链路，减少重复性人工操作，同时保留必要的质量审核、版权检查和发布确认环节。

该系统不是简单地让模型“一键生成视频”，而是将长流程拆解为多个可追踪阶段，通过结构化数据、代码校验、失败恢复、任务调度和结果复用降低自动化过程中的错误和成本。自动抓取、改写和发布功能仍需遵守目标平台的接口规范、版权要求和内容安全规则，当前属于后续演进方向。
...
![人生副本首页](./assets/workbench-homepage.png)

### 工作台阶段 05：提示词生成

![工作台阶段 05 提示词生成](./assets/workbench-stage05-prompts.png)
