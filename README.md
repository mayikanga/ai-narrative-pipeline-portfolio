...
流水线代码 / Schema / 适配器发生改动 → 回归测试与成本分析
```
...
文本流水线的阶段结果通过必要验收后，可以使用 `--generate-images` 将结构化结果交给生图流水线。生图流水线读取场景提示词、人物阶段设定、地点信息和 `referencePlan`，再执行角色参考图与场景图任务。

```mermaid
flowchart LR
    A["文本流水线验收通过"] --> B["场景提示词 + 人物设定 + referencePlan"]
    B --> C["生图任务标准化"]
    C --> D["角色阶段参考图"]
    D --> E["依赖调度与异步轮询"]
    E --> F["场景图生成"]
...
> 设计并实现一套将中文人生故事转换为连续视觉分镜的 AI 流水线，文本阶段验收通过后可直接进入 Node.js 生图任务调度；采用“模型生成 + 代码校验 + 定点修复”架构，通过稳定索引、原文哈希、JSON Schema 和阶段级 checkpoint 降低模型输出漂移与长流程重复调用成本；在代码改动后使用 10 篇固定案例进行逐阶段回归，记录 Token、缓存、耗时和费用，并部署 Vercel 可视化工作台。
