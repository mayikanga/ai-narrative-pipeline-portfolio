# AI 叙事视觉化端到端流水线与回归测试体系

> 将中文人生故事转换为连续视觉分镜，并用代码校验、阶段检查点和多案例回归测试控制大模型输出质量。

## 项目概览

这是一个面向 AI 内容生产的端到端工程化流水线。输入一篇中文人生故事，文本流水线先完成语义分块、场景拆分、人物/地点/主题物件设定、场景提示词生成、参考图规划和最终验收；验收通过后，结构化结果直接交给生图流水线，继续执行角色参考图和场景图生成。
...
三个部分不是平行的独立工具，而是一条连续链路：

```text
中文故事 → 文本流水线 → 结构化提示词与参考图计划
                         ↓ 验收通过
                  生图流水线 → 角色参考图与场景图
                         ↓
                回归测试与成本分析
...
- 最终验收覆盖 840 条场景级提示词；未来场景、非锚点场景和跨阶段主角等非法参考图引用均为 0；
...
文本阶段完成并通过验收后，输出会作为生图流水线的标准输入；生图流水线再根据提示词、人物阶段设定和 `referencePlan` 调度图片任务。
