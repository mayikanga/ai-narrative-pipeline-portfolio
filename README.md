# AI 叙事视觉化流水线与多案例回归测试体系

> 将中文人生故事转换为连续视觉分镜，并用代码校验、阶段检查点和多案例回归测试控制大模型输出质量。

## 项目概览

这是一个面向 AI 内容生产的工程化流水线项目。输入一篇中文人生故事，系统依次完成语义分块、场景拆分、人物/地点/主题物件设定、场景提示词生成、参考图规划和最终验收，输出可继续用于生图的连续视觉分镜数据。
...
- 建立 01～06 六阶段 AI 叙事处理流程；
- 使用 10 篇固定案例进行逐阶段回归；
- 阶段 05/06 全量 10 篇案例通过结构、语义、参考图和比例验收；
- 最终生成 840 条场景级提示词；
- 参考图规划中，未来场景、非锚点场景、跨阶段主角和超过两张参考图均为 0；
- 阶段 03 采用纯代码检验，避免不必要的模型请求；
- 支持模型生成、代码验收、失败项定点修复和最终复查；
- 支持 checkpoint、历史响应回放和阶段级恢复；
- 通过缓存复用和成本对比决定是否冻结新方案。
...
生图阶段不让模型自由决定所有参考图。代码根据人物是否可见、地点锚点是否合格、人生阶段和前序场景关系选择最多两张参考图，减少未来场景引用、人物错配和跨阶段外貌漂移。

### 5. 回放测试与成本分析

真实 API 适配器用于验证线上模型行为，回放适配器用于复用已保存的历史响应。两者分开后，可以在不产生新模型费用的情况下验证公共校验器、目录结构、恢复逻辑和基线读取逻辑。
...
> 设计并实现一套将中文人生故事转换为连续视觉分镜的 AI 流水线，覆盖语义分块、场景拆分、人物/地点/物件设定、提示词生成和参考图规划；采用“模型生成 + 代码校验 + 定点修复”架构，通过稳定索引、原文哈希、JSON Schema 和阶段级 checkpoint 降低模型输出漂移与长流程重复调用成本；构建基于 10 篇固定案例的逐阶段回归测试体系，记录 Token、缓存、耗时和费用，并部署 Vercel 可视化工作台。
